...
namespace WitchHuntStandard
{
    public class WitchHunt : ISearcher
    {
        ...
        {
            var enumerator = pattern.Split(' ').ToList().GetEnumerator();
                ...
                if (foundKeyword)
                {
                    switch (keyword)
                    {
                        ...
                        case Keywords.Read16:
                            return new IntPtr(BitConverter.ToInt16(Data.Span.Slice(resultPointer.ToInt32(), 2).ToArray(),0));

                        case Keywords.Read32:
                            return new IntPtr(BitConverter.ToInt32(Data.Span.Slice(resultPointer.ToInt32(), 4).ToArray(),0));

                        case Keywords.Read64:
                            return new IntPtr(BitConverter.ToInt32(Data.Span.Slice(resultPointer.ToInt32(), 8).ToArray(),0));

                        case Keywords.Tracerelative:
                            return new IntPtr(resultPointer.ToInt32() + 4 + BitConverter.ToInt32(Data.Span.Slice(resultPointer.ToInt32(), 4).ToArray(),0) + ImageBase.ToInt64());

                        case Keywords.Tracecall:
                            return new IntPtr(resultPointer.ToInt32() + 5 + BitConverter.ToInt32(Data.Span.Slice(resultPointer.ToInt32() + 1, 4).ToArray(),0) + ImageBase.ToInt64());
